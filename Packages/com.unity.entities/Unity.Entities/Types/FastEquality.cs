...
namespace Unity.Entities
{
    public static class FastEquality
    {
        ...
        private struct ManagedGetHashCodeImpl<T> where T : IEquatable<T>
        {
        ...
        private unsafe static TypeInfo CreateManagedTypeInfo(Type t)
        {
            ...
            if (t.IsClass) 
            {
                ...
                {
                    Layouts = null,
                    EqualFn = Delegate.CreateDelegate(typeof(TypeInfo.ManagedCompareEqualDelegate), equalsFn),
                    GetHashFn = Delegate.CreateDelegate(typeof(TypeInfo.ManagedGetHashCodeDelegate), getHashFn),
                    Hash = 0,
                };
            }
            else
...
#if !NET_DOTS
        public static int ManagedGetHashCode(object lhs, TypeInfo typeInfo)
        {
        ...
        private static bool TypeUsesDelegates(Type t)
        {
            ...
            if (!t.IsValueType && !typeof(IComponentData).IsAssignableFrom(t))
                return false;
#else
            if (!t.IsValueType)
                return false;
