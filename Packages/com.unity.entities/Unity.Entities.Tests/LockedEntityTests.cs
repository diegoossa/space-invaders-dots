...
namespace Unity.Entities.Tests
{
    public class LockedEntitiesTests : ECSTestsFixture
    {
        ...
        {
            var entity0 = m_Manager.CreateEntity(typeof(EcsTestData), typeof(EcsTestData2), typeof(EcsTestTag));
            var entity1 = m_Manager.CreateEntity(typeof(EcsTestData), typeof(EcsTestData2), typeof(EcsTestTag));
            var chunk = m_Manager.GetChunk(entity0);
            m_Manager.LockChunkOrder(chunk);

            // Can't change only one entity in locked order chunks
            Assert.Throws<InvalidOperationException>(() =>  m_Manager.AddComponentData(entity0, new EcsFooTest()));
            Assert.Throws<InvalidOperationException>(() =>  m_Manager.RemoveComponent<EcsTestData>(entity0) );
            
            // Can't add non-zero sized components to locked order chunks
            Assert.Throws<InvalidOperationException>(() => m_Manager.AddComponent(m_Manager.UniversalQuery, typeof(EcsFooTest)));
            
            // Can't remove non-zero sized components to locked order chunks
            Assert.Throws<InvalidOperationException>(() =>m_Manager.RemoveComponent(m_Manager.UniversalQuery, typeof(EcsTestData)));

            // Can't destroy only one entity in locked order chunks
            Assert.Throws<InvalidOperationException>(() =>  m_Manager.DestroyEntity(entity0));
        }
