...
namespace Unity.Scenes
{
    ...
    public class SubScene : MonoBehaviour
    {
        ...
        void AddSceneEntities()
        {
            ...
            var sceneHeaderPath = EntityScenesPaths.GetLoadPath(_SceneGUID, EntityScenesPaths.PathType.EntitiesHeader, -1);
            var headerExists = File.Exists(sceneHeaderPath);
            if (!headerExists || !IsSceneHeaderVersionValid(sceneHeaderPath))
            {
#if UNITY_EDITOR
                UnityEngine.Debug.LogWarning($"Sub-scene '{EditableScenePath}' has no valid header at '{sceneHeaderPath}'. Please rebuild the Entity Cache.", this);
#else
                UnityEngine.Debug.LogWarning($"Sub-scene '{name}' has no valid header. Please rebuild the Entity Cache.", this);
#endif
            }
#endif
        ...
        */
        ...
        static bool IsSceneHeaderVersionValid(string sceneHeaderPath)
        {
            using (var reader = new StreamBinaryReader(sceneHeaderPath))
                return reader.ReadInt() == SceneMetaDataSerializeUtility.CurrentFileFormatVersion;
        }
    }
}