...
namespace Unity.Scenes.Editor
{
    ...
    class LiveLinkConnection
    {
        ...
        void AddLiveLinkChangeSet(Hash128 sceneGUID, List<LiveLinkChangeSet> changSets, LiveLinkMode mode)
        {
            var liveLink = GetLiveLink(sceneGUID);
            ...
            if (updateLiveLink)
            {
                changSets.Add(LiveLinkDiffGenerator.UpdateLiveLink(editScene, sceneGUID, ref liveLink, sceneDirtyID, mode, _BuildSettings));
            }

            if (liveLink != null)
                _SceneGUIDToLiveLink[sceneGUID] = liveLink;
