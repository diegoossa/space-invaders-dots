...
namespace Unity.Entities.Serialization
{
    static class BoxedProperties
    {
            ...
            where TVisitor : IPropertyVisitor
        {
            ...
            if (resolved != null)
            {
                resolved.Accept(ref container, visitor, ref changeTracker);
            }
            else
    ...
    struct DictionaryContainer<TKey, TValue>
    {
    ...
    unsafe class PropertiesBinaryWriter : BasePropertyVisitor
    {
        ...
        protected override VisitStatus BeginCollection<TProperty, TContainer, TValue>
            (TProperty property, ref TContainer container, ref TValue value, ref ChangeTracker changeTracker)
        {
            if (typeof(System.Collections.IList).IsAssignableFrom(typeof(TValue)))
            {
                var type = typeof(TValue);
                Type tValue;
                if (type.IsGenericType)
                {
                    tValue = type.GetGenericArguments()[0];
                }
                else if(type.IsArray)
                {
                    tValue = type.GetElementType();
                }
                else
                {
                    throw new NotImplementedException("Unknown IList type");
                }

                if (tValue.IsGenericType && value is System.Collections.ICollection)
                {
                    throw new NotImplementedException(
                        $"Collections of collections are not supported. '{property.GetName()}' contains a nested collection '{tValue.Name}'. " +
                        $"Wrap this nested list in a struct as a workaround for now.");
                }
            ...
            {
                throw new NotImplementedException($"Only IList collection types are currently supported. Failed to handle '{property.GetName()}' of type '{typeof(TValue).Name}'");
            }
            ...
            (TProperty property, ref TContainer container, ref TValue value, ref ChangeTracker changeTracker)
        {
            if (typeof(System.Collections.IList).IsAssignableFrom(typeof(TValue)))
            {
                ...
                if (type.IsGenericType)
                {
                    ...
                    if (null == value)
                    {
                        list = (System.Collections.IList)Activator.CreateInstance<TValue>();
                    }
                    else
                    {
