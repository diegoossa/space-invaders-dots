...
namespace Unity.Scenes
{
    unsafe class AsyncLoadSceneOperation
    {
...
#else
        ResourceRequest         _ResourceRequestObjRefs;
#endif
        ...
        public void Update()
        {
...
#else
                        _ResourceRequestObjRefs = Resources.LoadAsync<ReferencedUnityObjects>(_ResourcesPathObjRefs);
                        _LoadingStatus = LoadingStatus.WaitingForResourcesLoad;
#endif
                    }
                    else
            ...
            {
#if ENABLE_SUBSCENE_IMPORTER
                var objRefs = _ResourceObjRefs;
#else
                if (!_ResourceRequestObjRefs.isDone)
                    return;

                if (!_ResourceRequestObjRefs.asset)
                {
                    _LoadingFailure = $"Failed to load object references resource '{_ResourcesPathObjRefs}'";
                    _LoadingStatus = LoadingStatus.Completed;
                    return;
                }
                var objRefs = _ResourceRequestObjRefs.asset as ReferencedUnityObjects;
#endif
