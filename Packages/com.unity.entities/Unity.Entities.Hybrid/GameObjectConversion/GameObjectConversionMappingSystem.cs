...
class GameObjectConversionMappingSystem : ComponentSystem
{
    ...
    Entity CreateDstEntity(UnityObject uobject, int serial)
    {
        ...
        #endif
        {
            ...
            if ((flags & k_EntityGUIDMask) != 0)
                m_DstManager.SetComponentData(entity, uobject.ComputeEntityGuid(serial));
    ...
    void ClearIncrementalConversion(GameObject gameObject)
    {
        ...
        m_JournalData.RemoveForIncremental(instanceId);
    }
...
#if PREFAB_IN_CONTEXT_EDITING
            sceneCullingMask = UnityEditor.GameObjectUtility.ModifyMaskIfGameObjectIsHiddenForPrefabModeInContext(
