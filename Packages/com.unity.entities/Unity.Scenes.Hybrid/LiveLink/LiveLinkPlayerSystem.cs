...
namespace Unity.Scenes
{
    ...
    class LiveLinkPlayerSystem : ComponentSystem
    {
        ...
        unsafe void ReceiveEntityChangeSet(MessageEventArgs args)
        {
            ...
            if (LiveLinkMsg.IsDebugLogging)
            {
                string msg = $"Receive EntityChangeSet: {args.data.Length} Object Guids:";
                foreach (var guid in resourcePacket.GlobalObjectIds)
                    msg += "\n" + guid.AssetGUID;
                LiveLinkMsg.LogInfo(msg);
            }
        ...
        void ResetGame()
        {
            LiveLinkMsg.LogInfo("Reset Game");
        ...
        protected override void OnUpdate()
        {
            ...
            if (!m_DidRequestConnection)
            {
                ...
                LiveLinkMsg.LogInfo("Connecting LiveLink");
                PlayerConnection.instance.Send(LiveLinkMsg.ConnectLiveLink, World.GetExistingSystem<SceneSystem>().BuildSettingsGUID);
            }
            ...
            {
                LiveLinkMsg.LogInfo("Applying changeset");
