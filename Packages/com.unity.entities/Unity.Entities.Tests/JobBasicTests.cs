...
namespace Unity.Entities.Tests
{
    ...
    public class JobBasicTests : ECSTestsFixture
    {
...
#if NET_DOTS    // TODO: Don't have the library in the editor that grants access.
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Write) != 0);
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Read) == 0);
#endif
            }
        }
        ...
        public struct SimpleAdd : IJob
        {
            ...
            public void Execute()
            {
#if NET_DOTS    // Don't have the limited library in the editor.
                AssertOnThread((input.m_Safety.version & AtomicSafetyHandleVersionMask.Write) == 0);
                AssertOnThread((input.m_Safety.version & AtomicSafetyHandleVersionMask.Read) != 0);
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Write) != 0);
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Read) == 0);
#endif
                for (int i = 0; i < N; ++i)
        ...
        public struct SimpleParallelFor : IJobParallelFor
        {
            ...
            {
#if NET_DOTS    // Don't have the limited library in the editor.
                AssertOnThread((a.m_Safety.version & AtomicSafetyHandleVersionMask.Write) == 0);
                AssertOnThread((a.m_Safety.version & AtomicSafetyHandleVersionMask.Read) != 0);
                AssertOnThread((b.m_Safety.version & AtomicSafetyHandleVersionMask.Write) == 0);
                AssertOnThread((b.m_Safety.version & AtomicSafetyHandleVersionMask.Read) != 0);
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Write) != 0);
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Read) == 0);
#endif
                result[i] = a[i] + b[i];
...
#if NET_DOTS    // Don't have the library in the editor that grants access.
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Write) != 0);
                AssertOnThread((result.m_Safety.version & AtomicSafetyHandleVersionMask.Read) == 0);
                AssertOnThread((threadMap.m_Safety.version & AtomicSafetyHandleVersionMask.Write) != 0);
                AssertOnThread((threadMap.m_Safety.version & AtomicSafetyHandleVersionMask.Read) == 0);
#endif
            }
        }
        ...
        public struct HashWriterJob : IJob
        {
            ...
            {
#if NET_DOTS    // Don't have the limited library in the editor.
                Assert.IsTrue((result.m_Safety.version & AtomicSafetyHandleVersionMask.Write) != 0);
                Assert.IsTrue((result.m_Safety.version & AtomicSafetyHandleVersionMask.Read) == 0);
#endif
                for (int i = 0; i < N; ++i)
                {
                    result.TryAdd(i, 47);
