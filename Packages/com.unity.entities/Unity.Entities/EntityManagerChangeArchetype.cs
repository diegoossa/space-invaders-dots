...
namespace Unity.Entities
{
    ...
    public sealed unsafe partial class EntityManager
    {
        ...
        public void AddComponent(EntityQuery entityQuery, ComponentType componentType)
        {
            ...
            AddComponent(entityQuery.m_QueryData->MatchingArchetypes, entityQuery.m_Filter, componentType);
        }
        ...
        internal void AddComponent(UnsafeMatchingArchetypePtrList archetypeList, EntityQueryFilter filter, ComponentType componentType)
        {
            ...
            using (var chunks = ChunkIterationUtility.CreateArchetypeChunkArray(archetypeList, Allocator.TempJob, ref filter, m_ComponentJobSafetyManager))
            {
                if (chunks.Length == 0)
                    return;
        ...
        void AddSharedComponentData(NativeArray<ArchetypeChunk> chunks, int sharedComponentIndex, ComponentType componentType)
        {
        ...
        void RemoveComponent(NativeArray<ArchetypeChunk> chunks, ComponentType componentType)
        {
            if (chunks.Length == 0)
                return;
